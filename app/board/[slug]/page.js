    ...
    >
      ...
      {showRename && (
        <div onClick={() => setShowRename(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#22222c', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '24px', width: '360px' }}>
            <div style={{ fontSize: '15px', fontWeight: '500', color: '#f5f5fa', marginBottom: '6px' }}>Rename board</div>
      ...
      {showDeleteConfirm && (
        <div onClick={() => setShowDeleteConfirm(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#22222c', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '24px', width: '360px' }}>
            <div style={{ fontSize: '15px', fontWeight: '500', color: '#f5f5fa', marginBottom: '8px' }}>Delete board?</div>
            <div style={{ fontSize: '13px', color: '#888', marginBottom: '24px' }}>This will permanently delete <span style={{ color: '#ccc' }}>{board.name}</span> and all its cards. This cannot be undone.</div>
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
      ...
      {showTagManager && (
        <div onClick={() => setShowTagManager(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#22222c', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '24px', width: '400px', maxHeight: '80vh', overflowY: 'auto' }}>
            <div style={{ fontSize: '15px', fontWeight: '500', color: '#f5f5fa', marginBottom: '20px' }}>Manage labels</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
              {tags.length === 0 && <div style={{ fontSize: '13px', color: '#888' }}>No labels yet. Create one below.</div>}
              {tags.map(tag => (
                <div key={tag.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: '#1c1c24', borderRadius: '8px', border: '0.5px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <button onClick={() => deleteTag(tag.id)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#666', fontSize: '14px', padding: '0' }}>✕</button>
                </div>
              ))}
            </div>
            <div style={{ borderTop: '0.5px solid rgba(255,255,255,0.06)', paddingTop: '16px' }}>
              <div style={{ fontSize: '11px', color: '#888', marginBottom: '10px', letterSpacing: '.06em' }}>NEW LABEL</div>
      ...
      <div style={{ width: '190px', flexShrink: 0, background: '#1c1c24', borderRight: '0.5px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', padding: '20px 0' }}>
        ...
        <div style={{ padding: '7px 16px', fontSize: '14px', color: '#888', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }} onClick={() => router.push('/dashboard')}>
          ⊞ All boards
        </div>
        <div style={{ padding: '16px 16px 6px', fontSize: '11px', color: '#666', letterSpacing: '.06em' }}>TRACK</div>
        {boards.map((b, i) => (
          <div key={b.id} onClick={() => router.push(`/board/${b.slug}`)} style={{ padding: '6px 16px', fontSize: '14px', color: b.slug === params.slug ? '#f5f5fa' : '#888', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div style={{ marginTop: 'auto', padding: '16px', borderTop: '0.5px solid rgba(255,255,255,0.05)' }}>
          <div onClick={() => { supabase.auth.signOut(); router.push('/login') }} style={{ fontSize: '13px', color: '#777', cursor: 'pointer' }}>⚙ Settings</div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
          <div style={{ fontSize: '14px', fontWeight: '500', color: '#f5f5fa', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#7F77DD', display: 'inline-block' }}></span>
            {board.name}
            <span style={{ fontSize: '12px', color: '#666', fontWeight: '400' }}>sorano.space/{board.slug}</span>
          </div>
          <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
            <button onClick={() => setShowTagManager(true)} style={{ padding: '5px 11px', borderRadius: '6px', border: '0.5px solid rgba(255,255,255,0.08)', background: 'transparent', fontSize: '12px', color: '#888', cursor: 'pointer' }}>Labels</button>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 11px', borderRadius: '6px', border: '0.5px solid rgba(255,255,255,0.08)' }}>
              <span style={{ fontSize: '12px', color: '#888' }}>{board?.is_public ? 'Public' : 'Private'}</span>
              ...
              {showMenu && (
                <div onClick={() => setShowMenu(false)} style={{ position: 'fixed', inset: 0, zIndex: 40 }}>
                  <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', right: 0, top: '32px', background: '#22222c', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '4px', minWidth: '160px', zIndex: 41 }}>
                    <div onClick={() => { setShowMenu(false); setRenameValue(board.name); setShowRename(true) }} style={{ padding: '8px 12px', fontSize: '13px', color: '#c0c0cc', cursor: 'pointer', borderRadius: '6px' }}>Rename board</div>
            ...
            <div key={col.key} style={{ flex: 1, borderRight: '0.5px solid rgba(255,255,255,0.05)', padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: '7px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                <div style={{ fontSize: '13px', color: '#888', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  ...
                  {col.label}
                  <span style={{ fontSize: '11px', color: '#666', background: '#22222c', padding: '1px 5px', borderRadius: '99px' }}>{cards.filter(c => c.status === col.key).length}</span>
                </div>
                <span onClick={() => setNewCard({ col: col.key, title: '', tag: '' })} style={{ fontSize: '15px', color: '#666', cursor: 'pointer', lineHeight: 1 }}>+</span>
              </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '5px', flexWrap: 'wrap', position: 'relative' }}>
                        <div onClick={e => { e.stopPropagation(); openPicker(pickerOpen ? null : card.id) }} style={{ cursor: 'pointer' }}>
                          {tag ? (
                            ...
                            <span style={{ fontSize: '10px', padding: '2px 7px', borderRadius: '99px', color: '#666', border: '0.5px dashed #555' }}>+ label</span>
                          )}
                        </div>
                        ...
                        {pickerOpen && (
                          <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', top: '22px', left: 0, background: '#2a2a34', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '4px', zIndex: 30, minWidth: '160px', boxShadow: '0 4px 20px rgba(0,0,0,0.4)' }}>
                            <div onClick={() => updateCardLabel(card.id, null)} style={{ padding: '6px 10px', fontSize: '12px', color: '#888', cursor: 'pointer', borderRadius: '5px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                            ...
                            <div style={{ borderTop: '0.5px solid rgba(255,255,255,0.06)', marginTop: '4px', paddingTop: '4px' }}>
                              {!pickerShowCreate ? (
                                <div onClick={() => setPickerShowCreate(true)} style={{ padding: '6px 10px', fontSize: '12px', color: '#888', cursor: 'pointer', borderRadius: '5px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                  <span>+</span> New label
                                </div>
                              ) : (
                        ...
                        <select onChange={e => moveCard(card.id, e.target.value)} value={card.status} style={{ fontSize: '11px', color: '#888', background: 'transparent', border: 'none', cursor: 'pointer', outline: 'none' }}>
                          {COLUMNS.map(c => <option key={c.key} value={c.key} style={{ background: '#22222c', color: '#c0c0cc' }}>{c.label}</option>)}
                        </select>
                      </div>
                      {count > 0 && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '3px', fontSize: '11px', color: isTop ? '#7F77DD' : '#777' }}>
              ...
              {newCard.col === col.key ? (
                <div style={{ background: '#22222c', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: '8px', padding: '10px 11px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <input autoFocus value={newCard.title} onChange={e => setNewCard({ ...newCard, title: e.target.value })} onKeyDown={e => e.key === 'Enter' && addCard(col.key)} placeholder="Card title..." style={{ background: 'transparent', border: 'none', outline: 'none', fontSize: '13px', color: '#f5f5fa', width: '100%' }} />
                  <select value={newCard.tag} onChange={e => setNewCard({ ...newCard, tag: e.target.value })} style={{ background: '#1c1c24', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: '4px', fontSize: '11px', color: '#888', padding: '3px 6px', outline: 'none' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    ...
                    <button onClick={() => setNewCard({ col: null, title: '', tag: '' })} style={{ fontSize: '12px', color: '#666', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>Cancel</button>
                  </div>
                </div>
              ) : (
                <div onClick={() => setNewCard({ col: col.key, title: '', tag: '' })} style={{ padding: '7px 11px', borderRadius: '8px', border: '0.5px dashed rgba(255,255,255,0.05)', fontSize: '13px', color: '#555', cursor: 'pointer' }}>+ Add card</div>
              )}
            </div>
          ))}
        </div>
